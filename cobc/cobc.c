...
static char		cob_cc[COB_SMALL_BUFF];			/* gcc */
...
void
cob_tree_cast_error (cb_tree x, const char *filen, const int linenum, const int tagnum)
{
...
size_t
cob_check_valid_name (char *name)
{
	size_t	n;
...
process_filename (const char *filename)
{
	...
	if (cob_check_valid_name (basename)) {
		...
		if (cb_listing_file) {
			...
			if (ppout) {
				...
				fprintf (cb_listing_file,
					"# Generated by %s.%d (Build date - %s)\n",
					PACKAGE_STRING, PATCH_LEVEL, cb_oc_build_stamp);
				fprintf (cb_listing_file,
					"# Environment :\n");
				fprintf (cb_listing_file,
					"#  TMPDIR : %s\n", cob_tmpdir);
