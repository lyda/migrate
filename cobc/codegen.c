...
output_call (struct cb_call *p)
{
	...
	for (l = p->args, n = 1; l; l = CB_CHAIN (l), n++) {
		...
		case CB_CALL_BY_REFERENCE:
			if (CB_NUMERIC_LITERAL_P (x) || CB_BINARY_OP_P (x)) {
				output_prefix ();
#ifdef HAVE_ATTRIBUTE_ALIGNED
				output ("unsigned char content_%d[8] __attribute__((aligned));\n", (int)n);
#else
				output ("unsigned char content_%d[8];\n", (int)n);
#endif
			}
			break;
		...
		case CB_CALL_BY_REFERENCE:
			if (CB_NUMERIC_LITERAL_P (x)) {
				...
				if (cb_fits_int (x)) {
					...
					output ("*(long long *)content_%d = ", (int)n);
	...
	if (!dynamic_link) {
		...
		if (!p->stmt1) {
			...
			output (";\n");
			output_line ("if (unlikely(unifunc.func_void == NULL))");
			output_indent_level += 2;
			output_stmt (p->stmt1);
			output_indent_level -= 2;
			output_line ("else");
...
/* RXW - Does not work correctly with debugger
...
*/
		...
		if (p->handler1 || p->handler2 || (p->file && CB_EXCEPTION_ENABLE (COB_EC_I_O))) {
			...
			if (p->handler2) {
				if (p->handler1 == NULL && p->file == NULL) {
...
output_file_allocation (struct cb_file *f)
{
	...
	if (f->organization == COB_ORG_RELATIVE || f->organization == COB_ORG_INDEXED) {
		output ("  static struct cob_file_key *%s%s = NULL;\n", CB_PREFIX_KEYS, f->cname);
	}
	output ("  static cob_file *%s%s = NULL;\n", CB_PREFIX_FILE, f->cname);
	output ("  static unsigned char %s%s_status[4];\n", CB_PREFIX_FILE, f->cname);
}
...
output_internal_function (struct cb_program *prog, cb_tree parameter_list)
{
	...
	if (prog->file_list) {
		...
		for (l = prog->file_list; l; l = CB_CHAIN (l)) {
			...
			if (CB_FILE (CB_VALUE (l))->linage) {
				i = 1;
			}
		}
		if (i) {
			output_line ("struct linage_struct *lingptr;");
		}
		output_newline ();
	}
...
*/
	...
	for (l = parameter_list; l; l = CB_CHAIN (l), i++) {
		...
		if (f->flag_any_length) {
			if (!anyseen) {
				anyseen = 1;
				output_line ("/* ANY LENGTH fields */");
			}
			output_line ("cob_field *anylen_%d;", i);
		}
	}
	if (anyseen) {
		output_newline ();
...
codegen (struct cb_program *prog, int nested)
{
	...
	if (!nested) {
		num_cob_genned = 0;
		loctime = time (NULL);
		strftime (locbuff, sizeof(locbuff) - 1, "%b %d %Y %T %Z",
			localtime (&loctime));
		output ("/* Generated by            cobc %s.%d */\n",
			PACKAGE_VERSION, PATCH_LEVEL);
		output ("/* Generated from          %s */\n", cb_source_file);
		output ("/* Generated at            %s */\n", locbuff);
		output ("/* OpenCOBOL build date    %s */\n", cb_oc_build_stamp);
		output ("/* OpenCOBOL package date  %s */\n", octardate);
		...
		output_storage ("/* Generated by            cobc %s.%d */\n",
			PACKAGE_VERSION, PATCH_LEVEL);
		output_storage ("/* Generated from          %s */\n", cb_source_file);
		output_storage ("/* Generated at            %s */\n", locbuff);
		output_storage ("/* OpenCOBOL build date    %s */\n", cb_oc_build_stamp);
		output_storage ("/* OpenCOBOL package date  %s */\n", octardate);
		output_storage ("/* Compile command         ");
		for (i = 0; i < cb_saveargc; i++) {
			output_storage ("%s ", cb_saveargv[i]);
		}
		output_storage ("*/\n\n");
