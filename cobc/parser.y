...
at_line_column:
  /* empty */			{ $$ = make_pair (cb_int1, cb_int1); }
		  ...
		  if (i == 0 || stack[i-1].token != VALUE)
		    {
		      /* unary negative */
		      l->next->item =
			make_binary_op (cb_zero, '-', l->next->item);
		      break;
		    }
		  /* fall through */
		case '+':
			...
			switch (token)
			  ...
			  case NEGATIVE:
			    op = ((token == POSITIVE) ? '>' :
				  (token == NEGATIVE) ? '<' : '=');
			    stack[i-1].value =
			      make_binary_op (stack[i-1].value, op, cb_zero);
			    break;
			  default:
			    stack[i-1].value =
