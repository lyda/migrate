...
cob_check_pos_status (int fret)
{
	...
	if (cob_current_module->crt_status) {
		if (COB_FIELD_IS_NUMERIC (cob_current_module->crt_status)) {
			...
			sprintf((char *)cob_current_module->crt_status->data, "%4.4d", fret);
		}
	}
	if (cob_current_module->cursor_pos) {
		...
		if (COB_FIELD_IS_NUMERIC (f) &&
			...
			if (f->size < 6) {
				sline *= 100;
				sline += scolumn;
				sprintf((char *)f->data, "%4.4d", sline);
			} else {
				sline *= 1000;
				sline += scolumn;
				sprintf((char *)f->data, "%6.6d", sline);
			}
		}
	}
}
...
cob_screen_puts (cob_screen *s, cob_field *f)
{
	...
	getyx (stdscr, y, x);
	line = SCREEN_LINE_POS (s);
	column = SCREEN_COLUMN_POS (s);
	if (line < 0) {
		line = y;
	}
	if (column < 0) {
